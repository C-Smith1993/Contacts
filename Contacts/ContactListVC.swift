...
class ContactListVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
            ...
            do {
                ...
                //print(readableJSON)
                    ...
                    for i in 0..<readableJSON.count{
                        ...
                        if let firstnameString = item["firstname"] as? String{
                        firstName = firstnameString
                        
                        if let surnameString = item["surname"] as? String{
                        surname = surnameString
                        
                        if let titleString = item["title"] as? String{
                        contactTitle = titleString
                        
                        if let addressString = item["address"] as? String{
                        address = addressString
                        
                        if let phoneNoString = item["phoneNumber"] as? String{
                        phoneNo = phoneNoString
                        
                        if let emailString = item["email"] as? String{
                        email = emailString
        ...
        // MARK: - Save the Retrieved Data Using CoreData
    func saveToCoreData(){
    ...
    (UIApplication.shared.delegate as! AppDelegate).saveContext()
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            firstNameLabel.text = contacts[indexPath.row].firstName + " \(surname)"
